...
class CustomWebsiteBlog(WebsiteBlog):
    ...
    def blog_post(self, blog, post, tag_id=None, page=1, enable_editor=None, **kwargs):
        ...
        """
        _logger.info(f"Here is the data we received: {kwargs}")
        ...
        try:
            ...
            # for b in all_blogs:
            #     _logger.info(f"Blog ID={b.id}, Name='{b.name}'")
            # for bp in all_blog_posts:
            #     _logger.info(f"Blog ID={bp.id}, Name='{bp.name}'")

            blog_space = blog_slug.replace('-', ' ')
            blog_hyphen = blog_slug.replace(' ', '-')
            _logger.info(f"Transformed blog slug to name format: '{blog_slug}'")

            blog_record = request.env['blog.blog'].search(['|', '|', ('name', 'ilike', blog_slug), ('name', 'ilike', blog_space), ('name', 'ilike', blog_hyphen)])

            if not blog_record:
                _logger.warning(f"No Blog found with slug='{blog}'")
                return request.not_found()
            
            for bg in blog_record:
                _logger.info(f"Found blog: ID={bg.id}, Name='{bg.name}'")

            post_name = post_slug.lower()  # Convert 'sierra-tarahumara' to 'Sierra Tarahumara'
            ...
            blog_post_space = post_name.replace('-', ' ')
            blog_post_hyphen = post_name.replace('-', ' ')
            # Search for the blog post using the slugs

            for idx, record in enumerate(blog_record):
                blog_post = request.env['blog.post'].search([
                    ('blog_id.id', '=', record.id), '|', '|',
                    ('name', 'ilike', post_name),
                    ('name', 'ilike', blog_post_space),
                    ('name', 'ilike', blog_post_hyphen)
                ], limit=1)

                if blog_post:
                    break  # Found a matching post, exit loop

                # If this is the last record and still nothing found, log and return 404
                if idx == len(blog_record) - 1:
    ...
    def blog(self, blog=None, tag=None, page=1, search=None, **opt):
        ...
        try:
            ...
            if blog:
                ...
                blog_space = blog.replace('-', ' ')
                blog_hyphen = blog.replace(' ', '-')
                _logger.info(f"Transformed blog slug to name format: '{blog}'")

                blog_record = request.env['blog.blog'].search(['|', '|', ('name', 'ilike', blog), ('name', 'ilike', blog_space), ('name', 'ilike', blog_hyphen)], limit=1)
